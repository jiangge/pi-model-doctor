...
`sync` discovers all models for a provider/channel from models.dev and lets the interactive UI select multiple models for this run. Canceling at any selection step exits with `not-persisted` and does not create a proposal or write. In non-interactive mode, pass `--models model-a,model-b`; there is no implicit catalog-first-model selection. Sync writes one combined proposal with one backup and one atomic write, so the selected models are applied together. `sync --dry-run` never writes models.json, backups, or caches.

`add` always starts with a provider id: `add <provider-id> [endpoint-url] [model]`. The second positional value is an endpoint only when it is an HTTP(S) URL; otherwise it is the model id. URL-first syntax is not supported. If the endpoint is omitted, the provider must already exist in `models.json`, so its endpoint, API protocol, headers, authentication, and other transport fields remain authoritative. In the interactive UI, an omitted model selects from catalog candidates; non-interactive mode requires an explicit model id. A new channel can be created with `add providerA https://gateway.example/v1 <model>` or first initialized as provider-only with `add providerA https://gateway.example/v1`, then populated with `add providerA <model>` or `sync providerA`. An unlisted third-party channel can use an exact models.dev model as metadata-only; use `--metadata-provider <models.dev-provider>` when the model id is ambiguous. `configure providerA [endpoint-url] [model]` changes the existing provider endpoint, API protocol, and/or API-key reference; without a model it also synchronizes the API on Model Doctor-managed models while preserving user-owned model APIs. Without `--api`, protocol inference uses existing configuration, catalog/provider metadata, endpoint evidence, and only then weak model-name hints; unresolved cases default to `openai-completions`. Supported API values are `openai-completions`, `mistral-conversations`, `openai-responses`, `azure-openai-responses`, `openai-codex-responses`, `anthropic-messages`, `bedrock-converse-stream`, `google-generative-ai`, `google-vertex`, and `pi-messages`. For a direct channel/model add, a bare root URL is normalized to `/v1` only for `openai-completions` or `openai-responses`; existing paths, query strings, and fragments are preserved. Provider-only setup does not append `/v1` because no model type is available. Provider website metadata is not fetched automatically. API credentials should be references such as `$OPENAI_API_KEY`, `${OPENAI_API_KEY}`, `!command`, or `pi-auth:provider`; literal API keys are not persisted unless `--allow-literal-api-key` is explicitly supplied. Secrets are never printed. Use `--dry-run` to inspect a proposal without writing. Non-interactive writes require `--yes`; `--dry-run` takes precedence over `--yes`. `migrate` accepts an explicit `--to provider/model`, or presents destination candidates in UI mode; non-interactive mode requires `--to`.
...
The repository deliberately does not install or pin `@earendil-works/pi-coding-agent` in `devDependencies`. It declares Pi as an optional host peer and the development scripts resolve the Pi installation already available on your machine. If Pi cannot be found automatically, set `PI_HOST_PACKAGE` to the installed `@earendil-works/pi-coding-agent` package directory. This keeps local checks aligned with the Pi version that will actually load the extension; it does not add the development helper to the published package.
