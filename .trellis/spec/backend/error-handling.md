...
- Slash-command handlers catch errors at the boundary and show `model-doctor: <message>` through `ctx.ui.notify`; domain modules throw typed errors instead of printing. Error messages, findings, plans, and warnings pass through secret redaction before user output. `sync` is a single combined proposal: UI selection ends with an explicit Done choice, any UI cancellation at any selection step returns `Status: not-persisted` without a proposal or write, headless mode requires `--models`, and all selected models share one authorization, backup, and atomic write. The package does not install a Pi host as a project development dependency; Pi supplies the optional peer at extension load time, while repository checks must resolve an installed host or fail with an actionable `PI_HOST_PACKAGE` message.
- Mutating commands in non-interactive modes require explicit `--yes`; `--dry-run` takes precedence and never persists config, backups, or caches. `add` is provider-id-first: `add <provider-id> [endpoint-url] [model]`; URL-first syntax is rejected, and omitting the endpoint requires an existing provider in `models.json`. The second positional value is an endpoint only when it is an HTTP(S) URL. `configure <provider-id> [endpoint-url] [model]` operates only on an existing provider and explicitly updates the supplied endpoint/API/API-key fields; provider-wide API updates synchronize Model Doctor-managed models but preserve user-owned model APIs. Synchronized model APIs remain managed with refreshed ownership snapshots and an `explicitFields` marker, so later `check`/`fix` preserves the explicit protocol instead of reverting it. `add --api-key` and `configure --api-key` accept references (`$ENV_VAR`, `${ENV_VAR}`, `!command`, or `pi-auth:`); literal keys are omitted unless explicit literal-key opt-in is supplied. All ten built-in Pi text API identifiers are accepted and listed in help/errors. Cache writes reject sensitive keys and use restrictive directory/file permissions. Successful default-path mutations refresh the Pi model registry and report `persisted-and-active`, `activation-failed`, or `persisted-reload-required`; custom model paths remain persistence-only until Pi reloads the matching runtime. An unlisted third-party channel can use an exact models.dev model match as metadata-only; its endpoint, API protocol, headers, authentication, and other transport fields remain channel-owned. Without `--api`, explicit configuration/catalog/endpoint evidence wins over weak model-name hints and unresolved cases default to `openai-completions`. For a direct channel/model add, a root URL receives `/v1` only for the resolved `openai-completions` or `openai-responses` family; explicit paths, query strings, fragments, Anthropic, and Google endpoints are preserved. Provider-only setup does not append `/v1` because no model type is available. `--metadata-provider` resolves duplicate model ids and `--api` is an explicit transport override that also controls root-URL normalization; no provider credentials are sent to or inferred for a third-party endpoint.
...
| URL-first add target | `DoctorError("invalid-target")`; provider-id-first syntax is required |
| add without endpoint URL | `DoctorError("invalid-target")` unless the provider id already exists in `models.json` |
| unlisted third-party channel with unique exact model metadata | metadata-only model proposal; preserve endpoint/API/headers/authentication |
...
- Do not append `/v1` before resolving the channel/model API family. For direct URL/model adds, normalize only a root URL; preserve explicit endpoint paths and keep query strings/fragments after the inserted `/v1`. Provider-only `add <provider-id> <endpoint-url>` setup has no model type and must remain unchanged. Pending provider-only metadata records the inferred API and endpoint hints; after model resolution, only still-inferred endpoint/API fields may be repaired. A changed endpoint/API is user-owned, is marked as blocked normalization state, and must remain untouched. Catalog root and `/v1` are equivalent only in both directions for OpenAI-compatible API families.
...
Preserve existing `ETag` and `Last-Modified` validators when a successful catalog response omits them so conditional refreshes remain effective. Reject credential-like or newline-bearing response validators as `invalid-catalog` before writing either catalog cache. A stale fallback returned during a dry-run must remain non-persisted in client memory so a later normal load cannot treat the dry-run result as a persisted cache.
