...
/model-doctor configure <provider-id> [endpoint-url] [model] [--api <protocol>] [--api-key <reference>] [--allow-literal-api-key] [--dry-run] [--yes]
...
ModelDoctor.proposeSetApi(input: SetApiInput): Promise<SetApiProposal>
ModelDoctor.applySetApi(proposal: SetApiProposal): Promise<{ backupPath?: string; plan: ChangePlan }>
...
`add` is provider-id-first: `add <provider-id> [endpoint-url] [model]`. The second positional value is an endpoint only when it is an HTTP(S) URL; otherwise it is the model id. URL-first syntax is rejected. When the endpoint is omitted, the provider id must already exist in `models.json`; existing endpoint, API, headers, authentication, and other transport fields remain authoritative. `AddInput` supports `providerId?: string` for explicit channel setup (`add providerA https://gateway.example/v1` or `add providerA https://gateway.example/v1 <model>`), `metadataProvider?: string` to disambiguate global models.dev metadata, `api?: PiApi` to explicitly select a channel transport protocol, and an internal `requireConfiguredProvider?: boolean` guard used by the command boundary. `SetApiInput` targets an existing provider and optionally an exact model; it can explicitly update endpoint, API protocol, and API-key reference, and provider-wide updates synchronize only Model Doctor-managed model APIs while preserving user-owned model APIs. Without `--api`, API resolution gives precedence to explicit configuration/catalog/endpoint evidence over weak model-name hints and defaults to `openai-completions`. The supported Pi API identifiers are `openai-completions`, `mistral-conversations`, `openai-responses`, `azure-openai-responses`, `openai-codex-responses`, `anthropic-messages`, `bedrock-converse-stream`, `google-generative-ai`, `google-vertex`, and `pi-messages`. `SyncInput` uses `modelIds: string[]` for headless selection and shares the same metadata-provider/API/credential safety rules as `AddInput`.
...
- models.dev endpoint: `PI_MODEL_DOCTOR_MODELS_DEV_URL` or `https://models.dev/api.json`. Custom endpoints use the same HTTPS/private-host policy; explicitly trusted private test infrastructure requires `PI_MODEL_DOCTOR_TRUSTED_ENDPOINT=1`. Provider API metadata must use HTTPS except that HTTP loopback URLs are accepted for models.dev entries representing local services; public HTTP, non-loopback private hosts, URL credentials, and credential query parameters remain invalid. Provider env metadata accepts uppercase alphanumeric/underscore names even when models.dev uses a leading digit; automatic API-key reference selection remains restricted to shell-safe letter-prefixed names. Model catalog keys and ids are case-sensitive exact strings for integrity/duplicate checks; case-folded discovery may report multiple case variants as ambiguous but must not discard either entry.
...
- `_piModelDoctor` is extension-owned metadata with `managed`, `source`, `lastCheck`, `autoRepair`, `version`, `managedFields`, `managedValues`, and optional `explicitFields`. Pending provider-only entries may additionally record `endpointNormalizationPending`, `endpointApiExplicit`, `endpointApiHint`, `endpointValueHint`, `endpointNormalizationBlocked`, and `endpointApiNormalizationBlocked`; these fields distinguish inferred transport values from later user edits. `explicitFields` records fields selected explicitly through provider-wide `configure`: those fields remain managed for future provider-wide synchronization, while `check`/`fix` preserve the explicit value instead of reverting it to catalog metadata.
...
- `models-cache.json` stores the complete normalized catalog; `providers-cache.json` stores schema-versioned provider summaries, adapter ids, environment-variable names, and independent capability signals. `policies-cache.json` stores a versioned `PolicyCatalog` used by capability resolution, including the Pi `0.82.1`/models.dev `api.json` normalized schema-version `1` compatibility baseline and observation date; invalid, old, sensitive, or insecurely-permissioned cache data is ignored and regenerated. Network normalization and normalized-cache validation share the same metadata shape rules. Model limits accept finite non-negative integers, including zero, while negatives, fractions, non-finite values, and non-numbers remain invalid. Model-level `interleaved` accepts the models.dev object form or a boolean, while unsupported primitives remain invalid; a boolean is preserved as catalog metadata and does not itself enable prompt/context/KV runtime capabilities. Reasoning options accept the models.dev `budget_tokens.min` sentinels `-1` and `0` only for that exact field/type combination; the capability resolver treats them as advisory invalid budget metadata rather than constructing a non-positive runtime budget. Reasoning option values may contain `null` as an upstream off/none sentinel, while runtime capability mapping consumes only non-empty strings. Cache writers reject sensitive object keys/values rather than persisting credentials. Cache capability output keeps prompt/context/KV control, pricing, usage, retention, and session-affinity semantics separate; unsupported runtime behavior is advisory rather than enabled. Capability results record confidence/source and cache adapters expose resolved, partial, advisory, or unsupported status independently for prompt/context/KV.
...
- `add` is provider-id-first and rejects URL-first syntax. The grammar is `add <provider-id> [endpoint-url] [model]`; without an endpoint the provider must already exist in `models.json`. An unlisted third-party channel can resolve an exact model id/name globally from models.dev as metadata-only. `add <provider-id> <endpoint-url> <model>` does the same with an explicit storage id. An endpoint URL given without a model id creates a provider-only entry (endpoint and inferred protocol, no model); only `add <provider-id> <endpoint-url>` provides this setup. Provider-only metadata records `endpointNormalizationPending` and the initial inferred `endpointApiHint`; once a model resolves the channel API, only an inferred root endpoint/API is normalized, while an explicit `--api`, explicit non-root path, or later user edit remains authoritative. Models are also attachable later with `add <provider-id> <model>` or `sync`. Duplicate catalog model ids are automatically disambiguated only when exactly one canonical provider can be established from models.dev metadata: prefer a model-declared native `@ai-sdk/<provider>` package, or, when no model declares an origin package, require exactly one candidate whose provider id matches its native `@ai-sdk/<provider>` package. The selected provider supplies metadata only. Otherwise `--metadata-provider` remains required. An explicit `--metadata-provider` always takes precedence. `--api` explicitly selects the Pi transport protocol when URL inference is insufficient. For direct channel/model adds, a root URL receives `/v1` only for `openai-completions` or `openai-responses`; Anthropic and Google families do not receive that suffix. Existing `/v1` and other explicit paths are preserved; query strings and fragments remain attached after an inserted `/v1`. Provider-only `add <provider-id> <endpoint-url>` setup does not append a suffix because no model type is available. An explicit `--api` controls the suffix decision, and an already configured channel endpoint remains authoritative. In metadata-only mode the channel's endpoint, API, headers, authentication, and unknown transport fields remain authoritative; models.dev provider identity is never copied into those fields. Official provider website metadata is not fetched implicitly, and any separately reviewed provider facts remain advisory until supplied through a supported metadata source.

- `configure <provider-id> [endpoint-url] [model]` operates only on an existing provider. It explicitly updates the requested endpoint/API/API-key fields, preserves omitted values, and when no model is selected synchronizes only Model Doctor-managed model APIs; user-owned model APIs remain unchanged. Provider-wide API synchronization keeps affected model APIs managed, refreshes their ownership snapshots, and records `explicitFields: ["api"]` so later `check`/`fix` does not undo the explicit protocol choice. All ten built-in Pi text APIs are accepted: `openai-completions`, `mistral-conversations`, `openai-responses`, `azure-openai-responses`, `openai-codex-responses`, `anthropic-messages`, `bedrock-converse-stream`, `google-generative-ai`, `google-vertex`, and `pi-messages`.
...
| Add without model id | provider-only entry only for `add <provider-id> <endpoint-url>`; model attach via `add <provider-id> <model>` or `sync` |
...
| Provider id without endpoint URL | provider id must already exist in `models.json`; existing endpoint/API/auth remain unchanged |
| URL-first add target | `invalid-target`; URL-first syntax is not supported |
| `configure` without an existing provider | `invalid-target`; no mutation |
...
| Model limit is a finite non-negative integer, including zero | normalize and preserve it consistently across network and cache validation |
| Model limit is negative, fractional, non-finite, or not a number | typed `ModelsDevError("invalid-catalog")`; do not cache the response |
| Reasoning option is `budget_tokens` with `min: -1` or `min: 0` | normalize/cache it, then resolve conservatively without a non-positive runtime budget |
| Another reasoning option uses a negative bound, or max is non-positive | typed `ModelsDevError("invalid-catalog")`; do not cache the response |
| Provider API is HTTPS or HTTP loopback | normalize/cache it; loopback HTTP represents an explicitly local provider |
| Provider API is public HTTP or non-loopback private HTTP | typed `ModelsDevError("invalid-catalog")`; do not cache the response |
| Reasoning option values contain non-empty strings or `null` | normalize/cache them; runtime mappings ignore `null` |
| Reasoning option values contain another primitive | typed `ModelsDevError("invalid-catalog")`; do not cache the response |
| Provider env metadata starts with an uppercase letter or digit and otherwise uses uppercase alphanumerics/underscores | normalize and preserve it in catalog/provider caches |
| Provider env metadata contains lowercase, whitespace, punctuation, or is empty | typed `ModelsDevError("invalid-catalog")`; do not cache the response |
| Two model ids differ only by case | preserve both exact entries; case-folded lookup returns ambiguous |
| A normalized model map key differs from its model id by any character/case | invalidate the cache; do not silently rewrite identity |
...
- Cache: local read, stale fallback, forced refresh, conditional headers, HTTP 304, policy schema validation, dry-run non-persistence, identical network/cache acceptance of boolean or object `interleaved` metadata while rejecting other primitives, acceptance of zero limits while rejecting negative, fractional, non-finite, or non-number limits, and exact acceptance of the `budget_tokens.min` sentinels `-1`/`0` while rejecting other negative reasoning bounds, and network/cache parity for HTTPS or HTTP-loopback provider APIs while rejecting other insecure/private URL forms, and acceptance of `null` reasoning-value sentinels while rejecting other non-string primitives, and network/catalog/provider-cache parity for uppercase env metadata including leading-digit names, plus exact case-sensitive model identity validation with ambiguity preserved at the lookup layer.
...
- Discovery: a duplicate global model id may auto-select exactly one metadata-declared canonical provider; all other ambiguous matches return a selection-required result. UI migration/add/sync flows present candidates, while headless flows require explicit targets/model lists. Add dry-runs and successful/no-op adds explicitly report success, persistence status, and the selected metadata provider when metadata-only resolution is used.
