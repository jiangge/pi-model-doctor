...
`sync` 从 models.dev 发现 provider/channel 的全部模型，交互式 UI 可在本次运行中选择多个模型。任何选择步骤取消都会以 `not-persisted` 结束，不创建 proposal，也不写入配置。非交互模式必须通过 `--models model-a,model-b` 显式指定；不会隐式选择 catalog 首项或全部模型。sync 是一次组合 proposal：一次确认、一次备份、一次原子写入，所有选中模型一起应用。`sync --dry-run` 不写入 models.json、备份或缓存。

`add` 始终以 provider id 开始：`add <provider-id> [endpoint-url] [model]`。第二个位置参数只有在它是 HTTP(S) URL 时才会被视为 endpoint，否则视为 model id；不再支持 URL-first 语法。如果省略 endpoint，provider 必须已经存在于 `models.json`，因此现有 endpoint、API 协议、headers、认证和其他传输字段保持不变。交互式 UI 中省略模型时会展示候选列表；非交互模式必须提供显式 model id。新渠道可以用 `add providerA https://gateway.example/v1 <model>` 创建，也可以先用 `add providerA https://gateway.example/v1` 建立 provider-only，再用 `add providerA <model>` 或 `sync providerA` 添加模型。未在 models.dev 登记的第三方渠道可以使用 models.dev 中精确匹配的模型作为 metadata-only；模型 id 有歧义时使用 `--metadata-provider <models.dev-provider>`。`configure providerA [endpoint-url] [model]` 可修改已有 provider 的 endpoint、API 协议和/或 API-key 引用；省略 model 时还会同步 Model Doctor 管理的模型 API，同时保留用户自有模型的 API。省略 `--api` 时，协议推断依次参考现有配置、catalog/provider metadata、endpoint，最后才使用弱模型名提示；无法判断时默认 `openai-completions`。支持的 API 值为 `openai-completions`、`mistral-conversations`、`openai-responses`、`azure-openai-responses`、`openai-codex-responses`、`anthropic-messages`、`bedrock-converse-stream`、`google-generative-ai`、`google-vertex` 和 `pi-messages`。一步添加渠道模型时，根 URL 仅在 `openai-completions` 或 `openai-responses` 下补全 `/v1`；已有路径、查询参数和 fragment 保持不变；provider-only 不补全 `/v1`。Provider 官网数据不会自动抓取。API 凭据应使用 `$OPENAI_API_KEY`、`${OPENAI_API_KEY}`、`!command` 或 `pi-auth:provider` 等引用；除非显式传入 `--allow-literal-api-key`，否则字面 API key 不会持久化。扩展不会打印 secret。使用 `--dry-run` 可预览而不写入；非交互写入需要 `--yes`，且 dry-run 优先级更高。`migrate` 接受显式 `--to provider/model`，UI 模式可展示目标候选；非交互模式必须提供 `--to`。
